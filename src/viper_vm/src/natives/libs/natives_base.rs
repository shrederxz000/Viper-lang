...
pub unsafe fn provide(built_in_address: &Address, vm: &mut VM) -> Result<(), Error> {
    ...
    natives::provide(
        ...
        |vm: &mut VM, addr: Address, should_push: bool, table: *mut Table| {
            ...
            match value {
                Value::Float(_) => {
                    vm.op_push(OpcodeValue::String("viper:f64".to_string()), table)?;
                }
                Value::Int(_) => {
                    vm.op_push(OpcodeValue::String("viper:i64".to_string()), table)?;
                }
                Value::String(_) => {
                    vm.op_push(OpcodeValue::String("viper:string".to_string()), table)?;
                }
                Value::Bool(_) => {
                    vm.op_push(OpcodeValue::String("viper:bool".to_string()), table)?;
                }
                Value::Type(_) => {
                    vm.op_push(OpcodeValue::String("viper:type".to_string()), table)?;
                }
                Value::Fn(_) => {
                    vm.op_push(OpcodeValue::String("viper:fn".to_string()), table)?;
                }
                Value::Native(_) => {
                    vm.op_push(OpcodeValue::String("viper:native".to_string()), table)?;
                }
                Value::Instance(i) => {
                ...
                Value::List(l) => {
                    vm.op_push(OpcodeValue::String("viper:list".to_string()), table)?;
                }
                Value::Null => {
                    vm.op_push(OpcodeValue::String("viper:null".to_string()), table)?;
                }
                Value::Any(any) => {
                    vm.op_push(OpcodeValue::String("viper:any".to_string()), table)?;
                }
            }
            Ok(())
        },
